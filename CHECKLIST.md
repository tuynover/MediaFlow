...
| 13  | 2026-08-24     | `MF-806`  | Hoàn thiện 100% Milestone 8 (M8 Baseline Freeze, Documentation README.md & KNOWN_LIMITATIONS.md, Clean-room Verification, và tạo Git Tag `mediaflow-baseline-v1`) | 🟢 Đã commit & push | `[2026-08-24] Commit #13: Dong bang phien ban MediaFlow Baseline v1 va tao Git Tag mediaflow-baseline-v1` (9086b34 - Tag: mediaflow-baseline-v1) |
...
### Milestone 6: Publish & Uncertain Recovery (M6) — 🟢 HOÀN THÀNH 100%
- [x] **`MF-601`**: Schema `publish_operations` lưu vết idempotency key & fingerprint.
- [x] **`MF-602`**: Deterministic copy publisher từ processed bucket ➔ delivery bucket.
- [x] **`MF-603`**: Giả lập lỗi mất response (FL-04 Response-loss simulation).
- [x] **`MF-604`**: Delivery verification (HEAD destination & metadata match).
- [x] **`MF-605`**: Operator reconcile endpoint `/api/v1/publish/:operationId/reconcile`.
- [x] **`MF-606`**: Integration test suite `test/integration/publish_reconcile.test.ts`.

---

### Milestone 7: Failure Lab, Ops CLI & UI Polish (M7) — 🟢 HOÀN THÀNH 100%
- [x] **`MF-701`**: Failure Lab demo fault configuration & Cờ bảo mật Fail Fast (`MEDIAFLOW_DEMO_MODE=true` cấm trong production).
- [x] **`MF-702`**: Xử lý 5 kịch bản lỗi giả lập FL-01 đến FL-05 (Interrupted upload, worker crash, corrupt output, publish timeout, cancellation).
- [x] **`MF-703`**: Outage runbook cho FL-06 (Stop/Start MinIO & Redis).
- [x] **`MF-704`**: Lệnh CLI Operator executable (`apps/ops`) cho các câu lệnh `list`, `inspect`, `retry`, `reconcile`, `watch`.
- [x] **`MF-705`**: Attention UI & Hướng dẫn hành động an toàn tiếp theo (Safe Next Action).
- [x] **`MF-706`**: Polish giao diện & tự động phục hồi kết nối.
- [x] **`MF-707`**: Fault integration test suite `test/fault/failure_lab.test.ts`.
